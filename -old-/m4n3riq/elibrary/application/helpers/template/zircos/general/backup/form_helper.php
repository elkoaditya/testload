...
if (!function_exists('view_loader')) {
	...
	function form_input_select1($d = '',$col_label1=3,$col_label2=8) {
		$data = cek_dataexists($d);
			$html = '
				   <div class="form-group">
                            <label class="col-sm-'.$col_label1.' control-label">'.$data['label'].'</label>
	...
	function form_input_textarea1($d = array(),$col_label1=3,$col_label2=8) {
		$data = cek_dataexists($d);
			$html = '
				  <div class="form-group">
					<label class="control-label col-md-'.$col_label1.'">'.$data['label'].'</label>
					<div class=" col-md-'.$col_label2.'">
														
						<textarea name="'.$data['name'].'" rows="5" class="form-control" />'.$data['value'].'</textarea>
