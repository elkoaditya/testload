...
function detikToPukul($value){
	//$time = gmdate("H:i:s", $value);
	$hour = (int)gmdate("H", $value);
	$minute = (int)gmdate("i", $value);
	$time = $hour." jam ".$minute." menit"; 

	return $time;
}

function cek_admin($admin_id, $role_admin, $data){
	
	$cek = $data;
	//if(($role_admin != 'super_admin')&&($role_admin != 'admin')){
		if(($data=='')&&($role_admin != 'super_admin')&&($role_admin != 'admin')){
			redirect(base_url(), $verifikasi);
		}else{
			if($data!=''){
				foreach($cek as $value_di=>$key){
					if(($role_admin != 'super_admin')&&($key['login_role_id']==1)){
						redirect(base_url(), $verifikasi);
					}
					/*if($key['login_id']!=$admin_id){
						redirect(base_url().'data/user_login/detail/'.$admin_id, $verifikasi);
					}*/
				}
			}
		}
