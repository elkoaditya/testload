    ...
    function verifikasi($send_data='') {
		...
		if(isset($send_data['username']))
		{
			...
			foreach ($query->result() as $arra1) {
				...
				if($arra1->role == 'sdm'){
					$role = 'guru';
				}elseif($arra1->role == 'siswa'){
					$role = 'user';
				}elseif($arra1->role == 'admin'){
					$role = 'super_admin';
				}
		...
		if(isset($data1['user']['id']))
		{
			...
			$query = $this->db->get('perpus_log_login');
			...
			if($ada==1){			
				$this->db->where('log_id',$data1['user']['id']);
				$this->db->update('perpus_log_login', $data_update);
			}else{
				$data_update['log_id'] = $data1['user']['id'];
				$this->db->insert('perpus_log_login', $data_update);
			}
			////////////////////////////////////
			
			// UPDATE DATA USER
			$data_user['perpus_sid']	= $data_update['log_sid'];
	 ...
	 function cek_siswa() {
		...
		if(isset($this->session->userdata['user'])){
			$this->db->where('perpus_sid', $this->session->userdata['user']['sid']);
