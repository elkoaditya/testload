...
class mod_login_admin extends CI_Model {
    ...
    function verifikasi($send_data='') {
		...
		if(isset($send_data['username']))
		{
			...
			$this->db->join('perpus_user_login_role role', 'role.login_role_id =  login.login_role_id', 'inner');
			
			$query = $this->db->get('perpus_user_login login');
			$a=0;
		...
		if(isset($data1['user']['id']))
		{
			...
			$this->db->update('perpus_user_login', $data_update);
	 ...
	 function cek_login() {
		...
		if(isset($this->session->userdata['user'])){
			...
			$query = $this->db->get('perpus_user_login');
