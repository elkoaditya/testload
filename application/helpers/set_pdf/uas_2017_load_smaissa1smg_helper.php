...
function header_2017_v2($row,$row_per_siswa)
{
	...
	<div id="header_1" class="header">
		<table width="100%" border="0" style="width: 100%;">
			<tr>
			  <td width="19%"	valign="top"><span class="style1">Nama Sekolah</span> </td>
			  <td width="1%"	valign="top"><span class="style1">:</span></td>
			  <td width="50%" 	valign="top"><span class="style1">'.APP_SCHOOL.'</span></td>
			  
			  <td width="16%" valign="top"><span class="style1">Kelas</span></td>
			  <td width="1%" valign="top"><span class="style1">:</span></td>
...
function tablepdf_2017_v3($resultset,$row,$row_per_siswa,$deskripsi_array,$header,$type_return=0,$page_sikap=0,$background = 0,$kkm=70)
{
	$mp_no = 0;
	$ktg_ascii = 64;
	$ktg_nama = NULL;
...
//     $html .= '<br/><br/>';
	...
	///////////////////////////////////////
	//cek nilai PAI = (Aqidah + Fiqih)/2
	$pai_nas_teori		= 0;
	$pai_nas_praktek	= 0;
	foreach ($resultset['data'] as $idx => $item){
		if(($item['mapel_id']==60)||($item['mapel_id']==61)){
			$pai_nas_teori 		= $pai_nas_teori + ($item['nas_teori']*0.5);
			$pai_nas_praktek 	= $pai_nas_praktek + ($item['nas_praktek']*0.5);
		}
		// fiqih
		if($item['mapel_id']==61){
			$pai_des_teori 		= $item['cat_teori'];
			$pai_des_praktek 	= $item['cat_praktek'];
		}
	}
	////////////////////////////////////
	for ($i = 1; $i <= 2; $i++) {
		...
		$html .= 		$header;
			...
			$html .=	head_table_nilai_2017_v2($type_return);
						...
						foreach ($resultset['data'] as $idx => $item)
						{
							/// tanpil tanpa ciri khusus
							if ($item['kategori_kode'] != 'KelD2'){
								
								///////////////////////////////////////
								//cek nilai PAI = (Aqidah + Fiqih)/2
								if (strpos( strtolower($item['mapel_nama']) , 'agama') !== false) { 
									/// teori
									$nilai_pred='';
									$item['nas_teori']	 = $pai_nas_teori;
									$item['cat_teori']	 = $pai_des_teori;
									if($item['nas_teori']>90){
										$nilai_pred = "A";
									}elseif($item['nas_teori']>79){
										$nilai_pred = "B";
									}elseif($item['nas_teori']>69){
										$nilai_pred = "C";
									}else{
										$nilai_pred = "D";
									}
									$item['pred_teori']	 = $nilai_pred;
									
									///praktek
									$nilai_pred='';
									$item['nas_praktek'] = $pai_nas_praktek;
									$item['cat_praktek'] = $pai_des_praktek;
									if($item['nas_praktek']>90){
										$nilai_pred = "A";
									}elseif($item['nas_praktek']>79){
										$nilai_pred = "B";
									}elseif($item['nas_praktek']>69){
										$nilai_pred = "C";
									}else{
										$nilai_pred = "D";
									}
									$item['pred_praktek']	 = $nilai_pred;
								}
								//////////////////////////////////////////////
...
function head_table_nilai_2017_v2($type=0)
{
	...
	$html .='
        ...
        <td class="thead-1 t-border " '.$set_rowspan.'  width="20%"><b>Mata Pelajaran</b></td>
        <td class="thead-1 t-border " width="7%"><b>Nilai</b></td>
        <td class="thead-1 t-border " width="9%" ><b>Predikat</b></td>
...
function table_interval_kkm_v3()
{
	$html ='
	...
	$html .='		<tr>
			...
			<td class="field-nilai t-border" align="center"> < 70 </td>
			<td class="field-nilai t-border" align="center"> 70 - 79</td>
			<td class="field-nilai t-border" align="center"> 80 - 90 </td>
...
function nilai_cirikhusus_2017_v1($resultset,$row,$row_per_siswa,$deskripsi_array,$header,$type_return=0,$page_sikap=0,$background = 0,$kkm=70){
	$html = ''; 
	$mode_range = 100;
	$html .= ' 
				<table id="t-nilai" >
					<tr>
						<td class="sub-kategori" width="20px"><b></b></td>
						<td class="sub-kategori"><b>Kelompok D (Ciri Khusus)</b></td>
					</tr>';
	for ($i = 1; $i <= 2; $i++) {
			$mp_no=1;
			
			if($i == 1){
				$keterangan_nilai = 'Pengetahuan';
			}else{
				$keterangan_nilai = 'Keterampilan';
			}
			$html .= ' 
					<tr>
						<td class="sub-kategori" width="20px"><b></b></td>
						<td class="sub-kategori"><b>'.$keterangan_nilai.'</b></td>
					</tr>
					<tr>
						<td></td>
						<td>
...
function prestasi_bta_2017_v1($resultset,$row,$row_per_siswa){
	$html = ''; 
	$mode_range = 100;
	$html .= ' 
				<table id="t-nilai" >
					<tr>
						<td class="sub-kategori" width="20px"><b></b></td>
						<td class="sub-kategori" align="center"><h3>DATA PRESTASI BACA TULIS AL QUR`AN</h3></td>
					</tr>';
	$html .= ' 
					<tr>
						<td class="sub-kategori" width="20px"><b></b></td>
						<td class="sub-kategori" align="center">';

	$html .='
						<table cellspacing="0" id="t-nilai" class="t-border" >
							<tr>
								<td class="thead-1 t-border " rowspan="2" width="3%"><b>No</b></td>
								<td class="thead-1 t-border " rowspan="2"><b>Materi Pelajaran</b></td>
								<td class="thead-1 t-border " colspan="2"><b>Nilai</b></td>
							</tr>
							<tr>
								<td class="thead-1 t-border " width="15%" ><b>Angka</b></td>
								<td class="thead-1 t-border " width="15%" ><b>Keterangan</b></td>
							</tr>';
	$i=1;
	$no=1;
	while($i<=5){
		if($i==1){
			...
			$html .='						
							<tr>';
			$html .=				"<td class=\"field-nilai t-border\" valign=\"top\" align=\"left\" colspan='4' ><b>A. Kefasihan Membaca</b></td>" . NL;
			$html .='						
							</tr>';
		}
		if($i==4){
			...
			$html .='						
							<tr>';
			$html .=				"<td class=\"field-nilai t-border\" valign=\"top\" align=\"left\" colspan='4' ><b>B. Tahfidz (Hafalan)</b></td>" . NL;
			$html .='						
							</tr>';
		}
		$html .='						
							<tr>';
		$html .=				"<td class=\"field-nilai t-border\" valign=\"top\" align=\"center\" >" . $no ."</td>" . NL;
		$html .=				"<td class=\"field-nilai t-border\" valign=\"top\" align=\"left\" >" . $row_per_siswa['bta_materi'.$i]."</td>" . NL;
		$html .=				"<td class=\"field-nilai t-border\" valign=\"top\" align=\"center\" >" . $row_per_siswa['bta_nilai'.$i]."</td>" . NL;
		$html .=				"<td class=\"field-nilai t-border\" valign=\"top\" align=\"left\" >" . $row_per_siswa['bta_ket'.$i] . "</td>" . NL;
		$html .='						
							</tr>';
		$i++;
		$no++;
	...
	$html .='						
							<tr>';
			$html .=				"<td class=\"field-nilai t-border\" valign=\"top\" align=\"left\" colspan='4' >Catatan : " . $row_per_siswa['bta_catatan']."</td>" . NL;
			$html .='						
							</tr>';
