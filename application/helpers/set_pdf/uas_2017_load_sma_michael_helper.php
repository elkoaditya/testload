...
function header_2017_v3($row,$row_per_siswa)
{
	$smster_nama = '2 / Genap';
	if($row['semester_nama'] == 'gasal'){
		$smster_nama = '1 / Gasal';
			...
			$html .= ' 
				<table id="t-nilai" >
					...
					<tr>
						...
						<td class="sub-kategori"><b>Kriteria Ketuntasan Minimal (KKM) = 60</b></td>
					</tr>
					<tr>
						<td></td>
						<td>
								...
								foreach ($deskripsi_pelajaran['data'] as $deskripsi)
								{
									if (($deskripsi['mapel_id'] == $item['mapel_id']) && ($deskripsi['kategori_id'] == $item['kategori_id']) && ($deskripsi['grade'] == $row['kelas_grade']) && ($deskripsi['aspek_penilaian_id'] == 1) )
									{
										...
										{							
											...
											{
												$cetak_des = 1;
				$html .=						"<td class=\"field-deskripsi2 t-border\" valign=\"top\" align=\"left\">{$deskripsi['deskripsi']}</td>" . NL;
											}
										}
									}
								}
								...
								foreach ($deskripsi_pelajaran['data'] as $deskripsi)
								{
									if (($deskripsi['mapel_id'] == $item['mapel_id']) && ($deskripsi['kategori_id'] == $item['kategori_id']) && ($deskripsi['grade'] == $row['kelas_grade']) && ($deskripsi['aspek_penilaian_id'] == 2) )
									{
											...
											( $item['pred_praktek']=='D' && $deskripsi['kode']==4 )	)
										{
											...
											{
												$cetak_des = 1;
				$html .=						"<td class=\"field-deskripsi2t-border\" valign=\"top\" align=\"left\">{$deskripsi['deskripsi']}</td>" . NL;
											}
										}
									}
								}
...
function table_interval_kkm_v2()
{
	...
	$html .='		<tr>
			...
			<td class="field-nilai t-border" align="center"> < 60 </td>
			<td class="field-nilai t-border" align="center"> 60 ≤ n < 70</td>
			<td class="field-nilai t-border" align="center"> 70 ≤ n < 86 </td>
			<td class="field-nilai t-border" align="center"> 86 ≤ n ≤ 100</td>
		';	
...
function ketidakhadiran_2017_v1($ketidakhadiran)
{
	...
	if($ketidakhadiran['absen_s'] > 0){
		$absen_s = $ketidakhadiran['absen_s'];
	}
	if($ketidakhadiran['absen_i'] > 0){
		$absen_i = $ketidakhadiran['absen_i'];
	}
	if($ketidakhadiran['absen_a'] > 0){
		$absen_a = $ketidakhadiran['absen_a'];
	}
	$html ='
	<table cellspacing="0" class="t-border field-nilai" width="50%" >
        ...
        <tr>
            ...
            <td class="t-border field-nilai"> :  '.$absen_s .' hari</td>
        </tr>
        <tr>
            <td class="t-border field-nilai">Ijin</td>
            <td class="t-border field-nilai"> :  '.$absen_i .' hari</td>
        </tr>
        <tr>
            <td class="t-border field-nilai">Tanpa Keterangan</td>
            <td class="t-border field-nilai"> : '.$absen_a .' hari</td>
        </tr>
    </table>';
	return $html;
}
