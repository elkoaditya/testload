	...
	<div id="header_1" class="header">
		<table width="100%" border="0" style="width: 100%;">
			...
			<tr>
			  ...
			  <td valign="top"><span class="style1">:</span></td>
			  <td valign="top"><span class="style1" colspan="4"><em>'.$row_per_siswa['siswa_nama'].'</em></span></td>
	...
	for ($i = 1; $i <= 2; $i++) {
		...
		$html .= 		$header;
			...
			//{
							...
							{
								$cetak_des = 1;
		$html .=						"<td class=\"field-deskripsi t-border\" valign=\"top\" align=\"left\">{$item['cat_teori']}</td>" . NL;
							}else{
								foreach ($deskripsi_pelajaran['data'] as $deskripsi)
								{
									if (($deskripsi['mapel_id'] == $item['mapel_id']) && ($deskripsi['kategori_id'] == $item['kategori_id']) && ($deskripsi['grade'] == $row['kelas_grade']) && ($deskripsi['aspek_penilaian_id'] == 1) )
			...
			//}else{ 
							...
							{
								$cetak_des = 1;
		$html .=						"<td class=\"field-deskripsi t-border\" valign=\"top\" align=\"left\">{$item['cat_praktek']}</td>" . NL;
							}else{
...
function tablepdf_2017_v4($resultset,$row,$row_per_siswa,$deskripsi_array,$header,$type_return=0,$page_sikap=0,$background = 0,$kkm=70)
{
	$mp_no = 0;
	$ktg_ascii = 64;
	$ktg_nama = NULL;
	$jml_row = 0;
	$nilai_catatan_walikelas = 0;
	$total_nilai_mapel = 0;
	$jumlah_mapel = 0;
...
//     $html .= '<br/><br/>';
			...
			//$html .=        		"<td class=\"field-nilai t-border\" valign=\"top\" align=\"center\"> ".$i." ddfd </td>" . NL;
							...
							if ($item['nipel_teori'])
							{
								if ($item['nas_teori']>0)
								{
									...
									/// catatan nilai walikelas
									$total_nilai_mapel = $total_nilai_mapel + $cetak_nas_teori;
	...
	if($i == 2){	
	  ...
	  if($type_return == 2){
		  $nilai_catatan_walikelas = round($total_nilai_mapel / $jumlah_mapel,0);
...
function head_table_nilai_2017_v2($type=0)
{
	...
	$html .='
        ...
        <td class="thead-1 t-border " width="10%"><b>Nilai</b></td>
        <td class="thead-1 t-border " width="10%" ><b>Predikat</b></td>
...
function table_interval_kkm_v2()
{
	...
	$html .='		<tr>
			...
			<td class="field-nilai t-border" align="center"> 76 ≤ n < 91 </td>
			<td class="field-nilai t-border" align="center"> 91 ≤ n < 100</td>
		';			
...
function table_interval_kkm_v3()
{
	...
	$html .='		<tr>
			...
			<td class="field-nilai t-border" align="center"> 80 - 89 </td>
...
function tableextra_2017_v4($ekskul_result,$prestasi_result,$row_per_siswa,$nilai_catatan_walikelas,$row, $color_menu='')
{ 
	$html = '
	<table id="t-nilai" width="100%" >   
         <tr>
...
function ekstrakurikuler_2017_v2($ekskul_result,$color_menu='')
{
	$html='
      <table cellspacing="0" class="t-border" width="100%" >
        <tr>
        	<td  class="t-border field-nilai '.$color_menu.'" align="center" valign="middle" width="20px"> <b>No</b></td>
            <td  class="t-border field-nilai '.$color_menu.'" align="center" valign="middle" width="26%"> <b>Kegiatan Ekstrakurikuler</b></td>
...
function catatan_walikelasv2($row_per_siswa , $nilai_catatan_walikelas,$row,$ekskul_result)
{
	if ((strtolower($row["semester_nama"]) == "genap"))
	{
		$keterangan_walikelas = array(
			...
			'1' =>	'Sangat bagus, terus raih kesempurnaan prestasi dan yakinlah dengan kesungguhan berusaha menjadikan semua sempurna.',
					
			'2' =>	'Prestasi yang bagus, tingkatkan lebih baik lagi dan yakinlah dengan kesungguhan berusaha menjadikan semua sempurna.',
				
			'3' => 'Prestasi yang cukup bagus, tingkatkan lebih baik lagi dan yakinlah bahwa prestasi lebih baik dapat diraih.',

			'4' => 'Masih harus banyak belajar dengan giat dan sungguh-sungguh dan yakinlah prestasi akan dapat dicapai asal mau berusaha.',
		);
      ...
      <table cellspacing="0" class="t-border" width="100%" >
        <tr>
        	<td class="t-border field-nilai" align="left" valign="top" height="'.$height.'">'; 
				...
				elseif($nilai_catatan_walikelas>=80)
					$html.= $keterangan_walikelas['2'];
				elseif($nilai_catatan_walikelas>=70)
					$html.= $keterangan_walikelas['3'];
				else
					$html.= $keterangan_walikelas['4'];
...
function tanggapan_ortu_2017_v2($ekskul_result ,$row)
{
	...
	if ((strtolower($row["semester_nama"]) == "genap") && ($row['kelas_grade']!=12))
	{
	  if($row['kelas_grade']==10){
		...
		//TIDAK NAIK 
		// X IPS 1 NAMANYA LESTARI BERLIANA OKTAVIA S. 
		// X IPS 2 NAMANYA PONCO ARI KURNIAWAN
	  if(($row['id']==7973) || ($row['id']==8014))
		{
...
function ttd_2017_v2($data,$type_ttd = 0)
{
	$nip=0;
	if(strpos(APP_SCOPE, 'sman') !== false) 
	{
        ...
        <tr>
            ...
            <td valign="top">
                '.$data["kota_nama"].', '.$data["tanggal_uas_nama"].'<br/>
					Wali Kelas
					<p>
						<img src="'.APP_ROOT.'/content/sman8smg/ttd/'.$data['wali_id'].'.png" height="70" style="margin-top:-7; margin-bottom:-6; position: absolute; top: 5px; left: 5px; z-index: 1" /><br>';
	$html .='			<u>' . $data["wali_nama"] . '</u>';
				...
				<table>
					<tr>
						<td style="padding-left:-40px;width: 100%;" >
							...
							Kepala Sekolah
							<p>
								<img src="'.APP_ROOT.'/content/sman8smg/ttd/Pak-kepsek.PNG" height="120" style="position: absolute; left: 5px; z-index: 1; margin-top:-25px; margin-bottom:-40px; " />
								<img src="'.APP_ROOT.'/content/sman8smg/ttd/stamp.PNG" height="125" style="position: absolute; left: 5px; z-index: 1; margin-left:-215px; margin-top:-25px; margin-bottom:-40px; " />
								<br>';
				$html .='		<u>' . $data["kepsek_nama"] . '</u>';
				if($nip==1){
					$html .='		<br/>
