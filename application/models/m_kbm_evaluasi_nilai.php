...
//echo"EEEE";
	...
	function download() {
		...
		$sheet->setCellValue('C5', 'No Ujian');
		...
		$sheet->getStyle("AR2")->applyFromArray($styleGreen);
		$sheet->setCellValue("AS2"," = Nilai Bonus");
		...
		foreach ($d['soal_array'] as &$s0al):
			...
			/// URAIAN
			if($s0al['posisi_kd']==999){
				$show_posisi_kd = "Uraian";
			/// ISIAN SINGKAT
			}elseif($s0al['posisi_kd']==99){
				$show_posisi_kd = "Isian Singkat";
			}else{
				$show_posisi_kd = $s0al['posisi_kd'];
			}
			
			
			if( (($s0al['type']== 3) && ($d['evaluasi']['plus_uraian']==0)) || 
				(($s0al['type']== 2) && ($d['evaluasi']['plus_isian']==0)) ){
				$sheet->getColumnDimension($s0al['excol']['pilihan'])->setVisible(FALSE);
			}
			
			$sheet->setCellValue($s0al['excol']['pilihan'].'4', $show_posisi_kd);
			$sheet->setCellValue($cell_soal_pilihan, $jml_soal);
			
			if (($s0al['type']== 3)||($s0al['type']== 2)){
				$width_colom = 30;
			}else{
				$width_colom = 4;
			}
			$sheet->getColumnDimension($s0al['excol']['pilihan'])->setWidth($width_colom);
		...
		if(($d['evaluasi']['plus_isian']==1) || ($d['evaluasi']['plus_uraian']==1)){
			...
			$sheet->setCellValue($cell_nilai_pilgan, "Nilai Pilgan(".$d['evaluasi']['bobot_pilgan']."%)");
			$sheet->getColumnDimension($col_nilai_pilgan)->setWidth(14);
			$total_bobot = $total_bobot + $d['evaluasi']['bobot_pilgan'];
			
			if($d['evaluasi']['plus_isian']==1){
				$col_nilai_isian = excel_colnumber(++$col_no);
				$cell_nilai_isian = $col_nilai_isian.'5';
				$sheet->setCellValue($cell_nilai_isian, "Nilai Isian Singkat(".$d['evaluasi']['bobot_isian']."%)");
				$sheet->getColumnDimension($col_nilai_isian)->setWidth(14);
				$total_bobot = $total_bobot + $d['evaluasi']['bobot_isian'];
			}
			
			if($d['evaluasi']['plus_uraian']==1){
				$col_nilai_uraian = excel_colnumber(++$col_no);
				$cell_nilai_uraian = $col_nilai_uraian.'5';
				$sheet->setCellValue($cell_nilai_uraian, "Nilai Uraian(".$d['evaluasi']['bobot_uraian']."%)");
				$sheet->getColumnDimension($col_nilai_uraian)->setWidth(14);
				$total_bobot = $total_bobot + $d['evaluasi']['bobot_uraian'];
			}
			
			$col_nilai_total = excel_colnumber(++$col_no);
			$cell_nilai_total = $col_nilai_total.'5';
			$sheet->setCellValue($cell_nilai_total, "Nilai Total(".$total_bobot."%)");
		...
		foreach ($d['resultset']['data'] as $row):
			...
			foreach ($d['soal_array'] as $soal_no => $soal):
				...
				if(isset($d['jawaban'][$row['siswa_id']]['nisisljs_array'][$soal['id']]['pilihan'])){
					...
					/// ISIAN SINGKAT
					if ($soal['type']== 2) {
						$jawaban = base64_decode($d['jawaban'][$row['siswa_id']]['nisisljs_array'][$soal['id']]['jawaban']);
						// cek kata2 ambigu di isian
					...
					if($nama_file['data'][0]['pilihan_jml'] > 0){
						...
						$sheet->getStyle($cell_soal_pilihan.":".$cell_soal_pilihan)->getNumberFormat()->setFormatCode( PHPExcel_Style_NumberFormat::FORMAT_TEXT );
						//if(strlen($jawaban)>300){
						//	$jawaban='(* jawaban terlalu panjang)';
						//}
						//if( ($jawaban_jml<49) && ($user_siswa<50) ){
			...
			/// NILAI TOTAL
			$total_bobot = $d['evaluasi']['bobot_pilgan'];
			if($d['evaluasi']['plus_isian']==1){
				$total_bobot += $d['evaluasi']['bobot_isian'];
			}
			if($d['evaluasi']['plus_uraian']==1){
				$total_bobot += $d['evaluasi']['bobot_uraian'];
			}
			$rumus = '';
			if(($d['evaluasi']['plus_isian']==1)||($d['evaluasi']['plus_uraian']==1)){
				$sheet->setCellValue($col_nilai_pilgan.$rowexcel, $row['evaluasi_nilai_utama']);
				$rumus .=" (".$col_nilai_pilgan.$rowexcel." * ".$d['evaluasi']['bobot_pilgan']." / ".$total_bobot.")";
				
				if($d['evaluasi']['plus_isian']==1){
					$sheet->setCellValue($col_nilai_isian.$rowexcel, $row['evaluasi_nilai_isian']);
					$rumus .=" + (".$col_nilai_isian.$rowexcel." * ".$d['evaluasi']['bobot_isian']." / ".$total_bobot.")";
				}
				if($d['evaluasi']['plus_uraian']==1){
					$sheet->setCellValue($col_nilai_uraian.$rowexcel, $row['evaluasi_nilai_uraian']);
					$rumus .=" + (".$col_nilai_uraian.$rowexcel." * ".$d['evaluasi']['bobot_uraian']." / ".$total_bobot.")";
				}
				$sheet->setCellValue($col_nilai_total.$rowexcel, "=(  ".$rumus." )");
			}
		endforeach;
