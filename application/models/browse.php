...
function display_nama_kesiswa($row)
{
	...
	if($trigger==0){	
		if($row['waktu_absensi']==''){
			...
			'<div id="kerjakan_'.$row['id'].'"><button class="btn btn-warning" onclick="javascript:input_absensi('.$id.' , '.$evaluasi_mulai.')"><i class="icon-tag"></i> Absensi </button></div>';
		}else{
...
<script type="text/javascript">
	...
	function input_absensi( id , ltz ) {
		$.ajax({ //saves changes from Handsontable
		  url: "<?php echo base_url('kbm/evaluasi_tool/input_absensi'); ?>",
		  dataType: "json",
		  type: "POST",
		  data: 'user_id=<?php echo $user["id"];?>&evaluasi_id='+id+'&ltz='+ltz, //returns full array of grid data
		  ...
		  success: function (data) {
			...
			if(data.status==0){
				$('#status_kerjakan_'+id).html('<div style="background-color:#FF4444"> Absen aktif 30menit sebelum Jadwal Dimulai </div>');
			}
		  ...
		  error: function (data) {
			//location.reload();
			$('#status_kerjakan_'+id).html('<div style="background-color:#FF4444"> Gagal Absen, silahkan cek Internet</div>');
		  }
		});
	}
