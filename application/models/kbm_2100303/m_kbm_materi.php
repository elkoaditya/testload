...
class M_kbm_materi extends MY_Model
{
	...
	function filter_1($query)
	{
		...
		if (isset($r['tanggal_publish'])){
			if($r['tanggal_publish']!=''){
			$where_range_materi = "WHERE materi_mulai >= '".$r['tanggal_publish']." 00:00:00' AND materi_mulai <= '".$r['tanggal_publish']." 23:59:00' ";
			if ($d['user']['role'] != 'siswa'){
				$this->db->join('( 
							SELECT * 
							FROM `kbm_materi_kelas` '.$where_range_materi.' 
							GROUP BY materi_id
						) materi_kelas' ,'materi_kelas.materi_id= materi.id ', 'left');
			}
			//$this->md->like($r['tanggal_publish'], 'materi.tanggal_publish');
				$query['where_strings'] = "((materi.tanggal_publish >= '".$r['tanggal_publish']." 00:00:00' AND materi.tanggal_publish <= '".$r['tanggal_publish']." 23:59:00') OR 
			(materi_kelas.materi_mulai >= '".$r['tanggal_publish']." 00:00:00' AND materi_kelas.materi_mulai <= '".$r['tanggal_publish']." 23:59:00')) " ;
			}
		}
	...
	function filtering()
	{
		...
		if (isset($r['tanggal_publish'])){
			...
			//$this->md->like($r['tanggal_publish'], 'materi.tanggal_publish');
			$this->db->where(" (materi.tanggal_publish >= '".$r['tanggal_publish']." 00:00:00' AND materi.tanggal_publish <= '".$r['tanggal_publish']." 23:59:00') OR 
		(materi_kelas.materi_mulai >= '".$r['tanggal_publish']." 00:00:00' AND materi_kelas.materi_mulai <= '".$r['tanggal_publish']." 23:59:00')" ) ;
	...
	function query_2()
	{
		...
		if ($d['user']['role'] == 'siswa'):
			...
			$query['join'][] = array('kbm_materi_kelas materi_kelas', "materi_kelas.materi_id = materi.id AND materi_kelas.kelas_id = '".$d['user']['kelas_id']."'", 'left');
			//$query['join'][] = array('dprofil_siswa siswa'," materi_kelas.kelas_id = siswa.kelas_id AND siswa.id = '".$d['user']['id']."' ", 'left');
			//$query['join'][] = array('dakd_kelas kelas',"kelas.id = siswa.kelas_id AND siswa.id = '".$d['user']['id']."'  ", 'left');
			...
			$query['select'][] = 'baca.nilai';
			
			$query['select'][] = 'materi_kelas.materi_mulai';
			$query['select'][] = 'materi_kelas.materi_ditutup';
			$query['select'][] = 'materi_kelas.meeting_id';
			$query['select'][] = 'materi_kelas.password';
			$query['select'][] = 'materi_kelas.encrypted_password';
			...
			*/
			...
			if(APP_SCOPE=='smk_pika'){
				...
				$query['where_strings'] = "( ( ( ( materi_kelas.materi_mulai <= '".$now."' && (materi_kelas.materi_ditutup = '0000-00-00 00:00:00' || materi_kelas.materi_ditutup is NULL ) ) OR " ;
				$query['where_strings'] .= "( materi_kelas.materi_mulai <= '".$now."' && materi_kelas.materi_ditutup >= '".$now."' ) ) " ;
				$query['where_strings'] .= "AND ( materi_kelas.materi_mulai > '0000-00-00 00:00:00' ) ) OR" ;
				
				$query['where_strings'] .= "( ( ( materi.tanggal_publish <= '".$now."' && (materi.tanggal_tutup = '0000-00-00 00:00:00' || materi.tanggal_tutup is NULL ) ) OR " ;
				$query['where_strings'] .= "( materi.tanggal_publish <= '".$now."' && materi.tanggal_tutup >= '".$now."' ) ) " ;
				$query['where_strings'] .= "AND ( materi.tanggal_publish > '0000-00-00 00:00:00' ) ) )" ;
				
			}else{
				
				$query['where_strings'] = "( ( ( ( materi_kelas.materi_mulai <= '".$now."' && (materi_kelas.materi_ditutup = '0000-00-00 00:00:00' || materi_kelas.materi_ditutup is NULL ) ) OR " ;
				$query['where_strings'] .= "( materi_kelas.materi_mulai <= '".$now."' && materi_kelas.materi_ditutup >= '".$now."' ) )  " ;
				//$query['where_strings'] .= "( materi_kelas.materi_mulai = '0000-00-00 00:00:00' || materi_kelas.materi_mulai is NULL )   " ;
				$query['where_strings'] .= "AND ( materi.tanggal_publish = '0000-00-00 00:00:00' || materi_kelas.materi_mulai is NULL ) ) OR" ;
				
				$query['where_strings'] .= "( ( ( materi.tanggal_publish <= '".$now."' && (materi.tanggal_tutup = '0000-00-00 00:00:00' || materi.tanggal_tutup is NULL ) ) OR " ;
				$query['where_strings'] .= "( materi.tanggal_publish <= '".$now."' && materi.tanggal_tutup >= '".$now."' ) OR " ;
				$query['where_strings'] .= "( materi.tanggal_publish = '0000-00-00 00:00:00' || materi.tanggal_publish is NULL ) ) " ;
				$query['where_strings'] .= "AND ( materi_kelas.materi_mulai = '' || materi_kelas.materi_mulai = '0000-00-00 00:00:00' || materi_kelas.materi_mulai is NULL ) ) )" ;
			}
...
INSERT INTO
	...
	(
		...
		nama, cuplikan, pelajaran_id, pelajaran_nilai_id, konten_tipe, konten, lampiran, pertanyaan,
		registered, updated
	)

SELECT
	{$semester_id}, {$author_id}, {$editor_id},
	nama, cuplikan, {$newPelajaran_id}, {$pelajaran_nilai_id}, konten_tipe, konten, lampiran, pertanyaan,
	registered, updated
