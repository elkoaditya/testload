...
class M_option extends CI_Model {
	...
	function pelajaran_kkm($prefill = FALSE, $bidang = 'materi') {
		$view = cfguc_view('akses', 'kbm', $bidang);
		...
		$config = array(
				...
				'prefill' => $prefill,
				'query' => array(
						'select' => array('pelajaran.id', 'nipel.kkm'),
						'from' => 'dakd_pelajaran pelajaran',
						'join' => array(
								array('nilai_pelajaran nipel', 'nipel.pelajaran_id  = pelajaran.id', 'inner'),
								array('prd_semester semester', 'semester.id = nipel.semester_id', 'inner')
						),
						'where' => array(
								'pelajaran.aktif' => 1,
	...
	function modul_rpp($prefill = FALSE, $rpp_id=0) {
		$config = array(
				'value' => 'id',
				'label' => 'nama',
				'prefill' => $prefill,
