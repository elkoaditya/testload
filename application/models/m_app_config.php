...
class M_app_config extends CI_Model
{
	...
	function private_tgl($index = 0, $limit = 50)
	{
		...
		$select = array(
			...
			'tanggal_kelulusan'	=> 'nikel.tanggal_kelulusan',
	...
	function save_private_tgl()
	{
		...
		$post_kelulusan = $this->input->post('config_value_kelulusan');
		...
		foreach ($post_mid as $key => $value) {
			...
			if (array_key_exists($key,$post_kelulusan))
			{	$data['tanggal_kelulusan']  = $post_kelulusan[$key];	}
