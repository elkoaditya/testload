...
class Cm_nilai_pelajaran_k13 extends MY_Model
{
	...
	function expor()
	{
		...
		//return alert_info('Sedang diperbaiki sesuai penilaian kurikulum 2017-2018.', "nilai/pelajaran/");
		...
		foreach ($kelas_list as $_kls_id)
		{
			...
			//if(!empty($data_kelas2['teori']['data']))
				//excel_table_write($_sheet, $data_kelas2['teori']['data'], $cfg['table2.catatan.teori'],13, 'A');
			//if(!empty($data_kelas2['praktek']['data']))
				//excel_table_write($_sheet, $data_kelas2['praktek']['data'], $cfg['table2.catatan.praktek'],13, 'A');
	...
	function impor()
	{
		...
		for ($_sheet_index = 0; $_sheet_index < $sheet_jml; $_sheet_index++)
		{
			...
			while($jml_cat<10)
			{
				...
				if($sheet->getCell('GD'.$no_cat)->getValue()=='')
				{
					$_row3['jenis_nilai']	= 1;
					$_row3['kode']			= (int)$jml_cat;
				...
				{
					$_row2['id']			= $sheet->getCell('GD'.$no_cat)->getValue();
					$_row2['jenis_nilai']	= 1;
			...
			{
				$jml_cat++; $no_cat=$no_cat+2;
				if($sheet->getCell('GE'.$no_cat)->getValue()=='')
				{
					$_row3['jenis_nilai']	= 2;
					$_row3['kode']			= (int)$jml_cat;
				...
				{
					$_row2['id']			= $sheet->getCell('GE'.$no_cat)->getValue();
					$_row2['jenis_nilai']	= 2;
					$_row2['kode']			= (int)$jml_cat;
