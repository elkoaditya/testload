	...
	function expor()
	{
		...
		foreach ($d['resultset']['data'] as $_row)
		{
			...
			else
			{
				...
				$data[$_kls_id]['nama'] 		= $_row['kelas_nama'];
				$data[$_kls_id]['nama_wali'] 	= $_row['kelas_wali'];
				$data[$_kls_id]['kelas_grade'] 	= $_row['kelas_grade'];
				$data[$_kls_id]['wali_id'] 		= (int) $_row['kelas_wali_id'];
...
*/
	...
	function impor()
	{
		...
		for ($_sheet_index = 0; $_sheet_index < $sheet_jml; $_sheet_index++)
		{
			...
			{
				$data_materi[ $_nipel_id[$_sheet_index] ][ $_nikel_id[$_sheet_index] ][$_db_col[0]] = $sheet->getCell($_excel_cell)->getValue();
				//echo"bbb ".$data_materi[ $_nipel_id[$_sheet_index] ][ $_nikel_id[$_sheet_index] ][$_db_col[0]]." n ".$_db_col[0]." o ".$_nipel_id[$_sheet_index]." ".$_nikel_id[$_sheet_index]." ".$_sheet_index;
			}
		...
		{
			//nama materi
			$this->db->update('nilai_pelajaran_kelas', $data_materi[$_nipel_id[$_sheet_index] ][$_nikel_id[$_sheet_index] ], array('pelajaran_nilai_id' => $_nipel_id[$_sheet_index], 'kelas_nilai_id' => $_nikel_id[$_sheet_index] ) );
			//echo"a ";print_r($data_materi[$_nipel_id[$_sheet_index] ][$_nikel_id[$_sheet_index] ]);
		}
