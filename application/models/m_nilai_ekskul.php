...
class M_nilai_ekskul extends MY_Model {

	public function __construct() {
		parent::__construct(array());
		$this->dm['admin'] = cfguc_admin('akses', 'nilai', 'ekskul');
		$this->dm['view'] = cfguc_view('akses', 'nilai', 'ekskul');
		$this->dm['ekskul_terkait'] = $this->ekskul_terkait();
	}

	// dasar database

	function ekskul_terkait() {
		$d = & $this->ci->d;
		$dm = & $this->dm;
		$user = $this->ci->d['user'];
		$query = array(
	...
	function filter_2($query) {
		...
		if (isset($r['ekskul_id']) && $r['ekskul_id'] > 0)
			$query['where']['nixkul.ekskul_id'] = $r['ekskul_id'];
	...
	function query_2() {
		...
		$query = array(
				'select' => array(
						...
						'ekskul_aktif' => 'ekskul.aktif',
						'pembina_nama' => "trim(concat_ws(' ', pembina.prefix, pembina.nama, pembina.suffix))",
						'pembina_aktif_id' => 'ekskul.pembina_id',
				),
				'from' => 'nilai_ekskul nixkul',
				'join' => array(
						...
						array('dprofil_sdm pembina', 'nixkul.pembina_id = pembina.id', 'left'),
				),
				'order_by' => 'semester.id desc, ekskul.nama',
		);

		// filter akses

		if (!$dm['admin'])
			$query['where']['ekskul.aktif'] = 1;

		if (!$dm['view'])
			$query['where']['ekskul.pembina_id'] = $d['user']['id'];

		if ($d['user']['role'] == 'siswa'):
			$query['join'][] = array('nilai_siswa_ekskul nisisxkul', 'nixkul.id = nisisxkul.ekskul_nilai_id', 'inner');
	...
	function rowsub_siswa($nixkul_id, $index = 0, $limit = 50) {
		...
		$query = array(
				...
				'where' => array(
						'nisisxkul.ekskul_nilai_id' => $nixkul_id,
				),
	...
	function expor($nixkul_id) {
		...
		// deklarasi variabel
		//$excel_source = APP_ROOT.'content/'.strtolower(APP_SCOPE).'/template/nilai_ekstrakurikuler.blank.xlsx';
		//$_sheet = clone $excel_obj->getSheetByName('nilai_ekstrakurikuler');
		...
		if (TRUE):
			...
			$excel_row_offset = 8;
			$excel_source = APP_ROOT.'content/'.strtolower(APP_SCOPE).'/template/nilai_ekstrakurikuler.blank.xlsx';
			$subsql_nisisxkul = "(select * from nilai_siswa_ekskul where ekskul_nilai_id = {$nixkul_id}) nisisxkul";
			...
			$cfg = array(
					'row.nixkul' => array(
							'A3' => array(
									'ekskul_nama',
									'prefix' => "Daftar nilai ekstrakurikuler ",
									'suffix' => array(
											' ',
											'semester_nama',
											' ',
			...
			if($d['pembina_ybs'])
			{
				$query_nisisxkul['where'] =
							"nisis.semester_id ='".$d['row']['semester_id']."'".
							" AND nisisxkul.keikutsertaan IS NOT NULL ".
							" AND nisisxkul.keikutsertaan !=''"
							;	
				$cfg['kolom.invicible'] = array('B','H');
			}*/
		...
		if (empty($req_kelas_list))
			return alert_error("Pilihan daftar kelas yang hendak ditampilkan.", "nilai/ekstrakurikuler/impor/{$nixkul_id}");
		...
		if ($template_result['selected_rows'] == 0)
			return alert_error("Daftar siswa tidak ditemukan pada kelas yang dipilih.", "nilai/ekstrakurikuler/impor/{$nixkul_id}");
		...
		foreach ($kelas_list as $_kls_id):
			...
			$_sheet->getStyle("H{$_row_start}:H{$_row_end}")->applyFromArray($style['tengah']);

			// proteksi worksheet/lembarkerja

			$_cell_input = array(
					"F{$_row_start}:H{$_row_end}",
			);
		...
		return excel_output_2007($excel_obj, $d['row']['ekskul_nama'] . '.xlsx');
	}
	...
	function impor() {
		...
		if (TRUE):
			...
			$nisixkul_query = array(
					'from' => 'nilai_siswa_ekskul',
					'where' => array(
							'ekskul_nilai_id' => (int) $d['row']['id'],
					),
					'select' => array('id', 'siswa_nilai_id'),
			);
			$date = new DateTime();
			$sql_rekap = "
update nilai_ekskul nixkul

inner join (
select ekskul_nilai_id, count(*) jml
from nilai_siswa_ekskul
where ekskul_nilai_id = ?
	and nilai != ''
group by ekskul_nilai_id
) nisisxkul on nixkul.id = nisisxkul.ekskul_nilai_id

set 
	nixkul.siswa_jml = nisisxkul.jml,
	nixkul.modified = '".$date->format('Y-m-d H:i:s')."'
...
";
		...
		// ambil data nilai ekskul yg telah ada
		...
		for ($_sheet_index = 0; $_sheet_index < $sheet_jml; $_sheet_index++):
			...
			// CHECK EKSKUL
			$_ekskul_nilai_id = (int) $sheet->getCell('B5')->getValue();
			...
			for ($_row_no = $row_start; $_row_no <= $row_max; $_row_no++):
				...
				$_keterangan = str_replace("'","`",$_keterangan);
				
				if($d['admin'])
				{
					$_cell = 'H' . $_row_no;
					$_keikutsertaan = clean($sheet->getCell($_cell)->getValue());
				}
		...
		if($nixkul_id != $_ekskul_nilai_id)
		{	
			return alert_error("Maaf di larang menggunakan excel lama atau excel dari ekskul lain. Silahkan pakai excel terbaru");
		...
		if (!empty($nisixkul_delete))
			if($d['admin']):
				$this->db->where_in('id', $nisixkul_delete)->delete('nilai_siswa_ekskul');
			else:
				$this->db->update_batch('nilai_siswa_ekskul', $nisixkul_update, 'id');
			endif;
		// update
		...
		if (!empty($nisixkul_insert))
			$this->db->insert_batch('nilai_siswa_ekskul', $nisixkul_insert);
