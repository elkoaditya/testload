...
class Evaluasi extends MY_Controller
{
	...
	public function __construct()
	{
		parent::__construct(array(
			...
			'kbm/evaluasi/form_bobot_nilai'		 => array(
				'user'				 => array('sdm', 'admin'),
				'model'				 => 'm_option',
				'library'			 => 'form',
				'helper'			 => 'form',
				...
				'validasi-bobot'		 => array(
					array(
						array(
							'field'	 => 'bobot_pilgan',
							'label'	 => 'bobot nilai pilgan',
							'rules'	 => 'required|in_range[0-100]|as_int',
						),
						array(
							'field'	 => 'bobot_isian',
							'label'	 => 'bobot nilai isian singkat',
							'rules'	 => 'required|in_range[0-100]|as_int',
						),
						array(
							'field'	 => 'bobot_uraian',
							'label'	 => 'bobot nilai uraian',
							'rules'	 => 'required|in_range[0-100]|as_int',
						),
					),
				),
			...
			'kbm/evaluasi/form_uraian'		 => array(
				'user'				 => array('sdm', 'admin'),
				'model'				 => 'm_option',
				'library'			 => 'form',
				'helper'			 => 'form',
				...
				'validasi-umum'		 => array(
					array(
						array(
							'field'	 => 'nama',
							'label'	 => 'nama evaluasi',
							'rules'	 => 'clean|required|min_length[2]|max_length[1000]',
						),
						array(
							'field'	 => 'kkm',
							'label'	 => 'KKM',
				...
				'validasi-bentuk'	 => array(
					array(
						array(
							...
							'rules'	 => 'required|select[0;1;2;3;4;5]|as_int',
						),
					),
				),
			...
			'kbm/evaluasi/form_upload_dokumen'	 => array(
				'user'				 => array('sdm', 'admin'),
				'model'				 => 'm_option',
				'library'			 => 'form',
				'helper'			 => 'form',
	...
	public function browse($index = 0,$limit = 20)
	{
		...
		//$d['pengumuman_evaluasi']	= $this->m_app_config->get('pengumuman_evaluasi');
	...
	{
		$this->_set('kbm/evaluasi/form_upload_dokumen');
		$this->_form_prepare();

		$d = & $this->d;

		if (!$d['error'] && $d['post_request'])
			if ($this->m_kbm_evaluasi->save_dokumen())
				return redir("kbm/evaluasi/id/{$d['form']['id']}");
	...
	public function form_bobot_nilai()
	{
		$this->_set('kbm/evaluasi/form_bobot_nilai');
		$this->_form_prepare();
	...
	{
		$this->_set('kbm/evaluasi/form_uraian');
		$this->_form_prepare();
	...
	public function _form_prepare()
	{
		...
		$d = & $this->d;
		if($d['row']['id'] == 0){
			$d['author_ybs'] = ($d['user']['id'] == $d['row']['author_id'] || $d['row']['id'] == 0);
		}else{
			$d['author_ybs'] = (($d['user']['id'] == $d['row']['author_id'])||($d['user']['id'] ==  $d['row']['pelajaran_guru_id']) );
		}
		...
		if ($d['modit']['pelajaran']):
			...
			$d['opsi_kkm'] = $this->m_option->pelajaran_kkm(FALSE, 'evaluasi');
	...
	public function _publish_prepare()
	{
		...
		//$d['author_ybs'] = ($d['user']['id'] == $d['row']['author_id']);
		$d['author_ybs'] = (($d['user']['id'] == $d['row']['author_id'])||($d['user']['id'] ==  $d['row']['pelajaran_guru_id']) || $d['row']['id'] == 0 );
	...
	public function pdf() {
		...
		$d['resultset'] = $this->m_kbm_evaluasi_soal->browse(0, 500);
	...
	public function kartu_soal() {
		...
		$this->_set('kbm/evaluasi/kartu_soal');
		$this->_rowset('m_kbm_evaluasi', $d['request']['evaluasi_id'], $uri_evaluasi, 'evaluasi');
		$d['resultset'] = $this->m_kbm_evaluasi_soal->browse(0, 500);
		
		return $this->_pdf("Kartu_Soal_{$d['evaluasi']['nama']}_{$d['evaluasi']['id']}_{$d['evaluasi']['mapel_nama']}_{$d['evaluasi']['kategori_nama']}");
