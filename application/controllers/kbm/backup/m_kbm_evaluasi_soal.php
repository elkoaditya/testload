...
class M_kbm_evaluasi_soal extends MY_Model {
	...
	function save3_penilaian_siswa1() {
		...
		// $data['hit_count'] = $this->input->post('jml_soal');
		$jml_soal = 1;
		
		$pil_soal = json_encode(array("benar","salah"));
		// $data['cuplikan'] = (string) clean($pil_soal);
		// $data['pertanyaan'] = base64_encode($pil_soal);

		// baca pilihan
		// $data['pilihan']['kunci']['index'] 		= "a";
		// $data['pilihan']['kunci']['label'] 		= base64_encode("benar");
		// $data['pilihan']['pengecoh']['index'] 	= "b";
		// $data['pilihan']['pengecoh']['label'] 	= base64_encode("salah");
