...
if ($pengecoh_jml >= 2):
	...
	if (isset($row['pilihan']['kunci']) && is_array($row['pilihan']['kunci']) && !$post_request):
		foreach ($row['pilihan']['kunci'] as $opsi): 
			$pengecoh_index = "pengecoh-" . $opsi;
			$pilihan[$pengecoh_index] = $pilihan['label'];
		endforeach;
	endif;
    ...
    <div class="container">
			...
			<div id="tabel">
				...
				foreach ($input as $inp):
					...
					echo div('class="controls"', form_cell($inp, $row['pilihan']['kunci']));
					echo "</label></div>" . NL;
				endforeach;
