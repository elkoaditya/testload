...
class Evaluasi_ljs extends MY_Controller {
	...
	public function __construct() {
		parent::__construct(array(
				...
				'kbm/evaluasi_ljs/download_absensi' => array(
						'user' => array('sdm', 'admin'),
						'library' => 'pagination',
						'model' => 'm_option',
						'helper' => 'form',
				...
				'kbm/evaluasi_ljs/download_surveillance' => array(
						'user' => array('sdm', 'admin'),
						'library' => 'pagination',
						'model' => 'm_option',
						'helper' => 'form',
	...
	public function _rowset_evaluasi() {
		...
		$d['wali_kelas'] = ($d['user']['id'] == $d['evaluasi']['wali_id']);

		if (!$d['wali_kelas'] && !$d['author_ybs'] && !$d['pengajar_ybs'] && !$d['view'] && $d['user']['role'] != 'siswa')
			return alert_error("Anda tak diperbolehkan mengakses ljs evaluasiaa.".$d['evaluasi']['wali_id'], 'kbm/evaluasi');
	}
	...
	public function form() {
		...
		$pakta_integritas = $this->m_app_config->get('pakta_integritas');
		
		
		if( $this->input->get('simulasi')){
			if( $this->input->get('simulasi') == 'ok'){
				$token = 'disable';
				$pakta_integritas = 'disable';
			}
		}
		...
		if ($token == 'enable')
		{
			...
			if(($this->input->get('token'))&&($go_evaluasi == 0)){
				...
				if( $check_token["token"] == $this->input->get('token') ){
		...
		if($go_evaluasi == 1){
			...
			// cek pakta integritas
			if($pakta_integritas == 'enable')
			{
				if($d['pengerjaan_ljs']['pakta_integritas']==0){
					$url_pakta_integritas = base_url()."kbm/evaluasi_tool/pakta_integritas?id=".$this->input->get('id');
					$md5_id = MD5($this->input->get('id'));
					
					if($this->input->get('kode_id')==$md5_id){
						
					}else{
						return redir($url_pakta_integritas);
					}
				}
			}
			// end cek pakta integritas
			...
			{	
				...
				{	
					// BATAS LIMIT WAKTU pengakses
					$limit_akses = explode(':',$d['evaluasi']['limit_akses']);
					$la_jam 	= (int)$limit_akses[0];
					$la_menit 	= (int)$limit_akses[1];
					$tot_limit_akses = ($la_jam*3600) + ($la_menit*60);
					if($tot_limit_akses >0){
						$deadline = strtotime($d['evaluasi']['evaluasi_mulai']) + $tot_limit_akses;
						//$deadline	= strtotime("+15 minute");
						$now	= strtotime("now");
						if($deadline <= $now){
							return alert_error('Maaf dilarang mengerjakan karena lebih '.$la_jam.'jam '.$la_menit.' menit dari waktu dimulai.', $uri_evaluasi);
						}
					}
			...
			if ($d['post_request']){
				...
				if ($this->m_kbm_evaluasi_ljs->save_last()){
					...
					echo "<script type='text/javascript'> document.location = '".base_url()."kbm/evaluasi'; </script>";
...
/*
	...
	public function surveillance($index = 0) {
		...
		$this->d['resultset'] = $this->m_kbm_evaluasi_ljs->surveillance($index, 2000);
		//print_r($this->d['resultset']);
		
		$this->_view();
	}
	
	public function download_surveillance($index = 0) {
		$this->_set('kbm/evaluasi_ljs/download_surveillance');
		$d = & $this->d;
		$this->_rowset_evaluasi();
		
		$this->d['resultset'] = $this->m_kbm_evaluasi_ljs->surveillance($index, 2000);
		//print_r($this->d['resultset']);
		$this->m_kbm_evaluasi_ljs->download_surveillance();
		
	}
	
	public function download_absensi($index = 0) {
		$this->_set('kbm/evaluasi_ljs/download_absensi');
		$d = & $this->d;
		$this->_rowset_evaluasi();
		
		$this->d['resultset'] = $this->m_kbm_evaluasi_ljs->surveillance($index, 2000);
		
		return $this->_pdf("Absensi_{$d['evaluasi']['nama']}_{$d['evaluasi']['id']}_{$d['evaluasi']['mapel_nama']}_{$d['evaluasi']['kategori_nama']}");
	}
