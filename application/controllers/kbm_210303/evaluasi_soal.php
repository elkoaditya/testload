...
class Evaluasi_soal extends MY_Controller {
	...
	public function __construct() {
		parent::__construct(array(
				...
				'kbm/evaluasi_soal/form_kunci' => array(
						'library' => 'form',
						'helper' => 'form',
						'request' => array(
								'evaluasi_id' => 'as_int',
				...
				'kbm/evaluasi_ljs/download_kunci' => array(
						'user' => array('sdm', 'admin'),
						'library' => 'pagination',
						'model' => 'm_option',
	...
	public function form() {
		...
		if ($d['row']['id'] > 0){
			...
			$d['form']['isian'] 		= $d['row']['isian'];
			$d['form']['uraian'] 		= $d['row']['uraian'];
			...
			$d['form']['isian'] 		= $this->input->get_post('isian');
			$d['form']['uraian'] 		= $this->input->get_post('uraian');
		}
		...
		if ($d['post_request'] && !$d['error']):
			if ($this->m_kbm_evaluasi_soal->save_abc()):
				...
				if ($tambah):
					...
					if($d['form']['isian']==1){
						$param_isian = "&isian=1";
					}elseif($d['form']['uraian']==1){
						$param_isian = "&uraian=1";
					}
					
					$redir = "kbm/evaluasi_soal/form_abc?evaluasi_id={$d['form']['evaluasi_id']}".$param_isian."&posisi_kd={$d['form']['posisi_kd']}&redir={$d['form']['redir']}";
	...
	public function form_kunci() {
		$d = & $this->d;
		$tambah = (bool) $this->input->post('tambah');

		$this->_set('kbm/evaluasi_soal/form_kunci');
		$this->form->init('m_kbm_evaluasi_soal', 'kbm/evaluasi');
		...
		if (!$d['author_ybs'] && !$d['admin'])
			return alert_error("Anda tak diperbolehkan mengubah kunci evaluasi.", 'kbm/evaluasi');

		if ($d['evaluasi']['closed'])
			return alert_error("Tak dapat mengubah kunci karena telah ditutup.", "kbm/evaluasi/id/{$d['evaluasi']['id']}");

		if ($d['row'] == 0 && $d['evaluasi']['published'])
			return alert_error("Tak dapat menambah kunci karena telah dipublikasikan.", "kbm/evaluasi/id/{$d['evaluasi']['id']}");

		if ($d['semaktif']['id'] == 0)
			return alert_error("Tidak dapat kunci mengubah evaluasi pada masa jeda semester.", 'kbm/evaluasi');

		else if ($d['evaluasi']['id'] > 0 && $d['evaluasi']['semester_id'] != $d['semaktif']['id'])
			return alert_error("Evaluasi tersebut telah berlalu.", "kbm/evaluasi/id/{$d['row']['id']}");

		if ($d['post_request'] && !$d['error']):
			if ($this->m_kbm_evaluasi_soal->save_kunci()):
			

				if ($tambah):
					$redir = "kbm/evaluasi_soal/form_kunci?evaluasi_id={$d['form']['evaluasi_id']}&posisi_kd={$d['form']['posisi_kd']}&redir={$d['form']['redir']}";
	...
	public function download_kunci($index = 0) {
		$this->_set('kbm/evaluasi_ljs/download_kunci');
		$d = & $this->d;
		...
		//print_r($d['resultset']);
		//echo"<br>";
		$this->m_kbm_evaluasi_soal->download_kunci();
