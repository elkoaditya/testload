...
class Siswa extends MY_Controller
{
	...
	public function __construct()
	{
		parent::__construct(array(
			...
			'data/profil/siswa/id'		 => array( 
				'model'		 => array('m_app_config', 'm_option', 'm_kbm_evaluasi', 'm_kbm_materi'), 
				'library'	 => 'pagination',
				'helper'	 => 'form',
				'request'	 => array( 
					'tglwaktu'		 	=> 'clean',
			...
			'data/profil/siswa/print_excel_siswa'	 => array(
				'user'		 => array('admin'),
				'model'		 => array('m_option', 'm_data_user'),
				'library'	 => array('form', 'PhpThumbFactory'),
				'helper'	 => array('form','excel'),
			),
			'data/profil/siswa/reset_password_back'	 => array(
				'user'		 => array('admin','sdm'),
				'model'		 => array('m_app_config_plus', 'm_data_user'),
			),
			'data/profil/siswa/manual_reset_password_siswa'	 => array(
				'user'		 => array('admin'),
				'library'	 => 'form',
				'helper'	 => 'form',
	...
	public function id($id = 0)
	{
		if(THEME=='material_admin'){
			...
			$this->m_dprofil_siswa->rowsub($id);
				...
				$d = & $this->d;
				// $d['request']['tglwaktu'] = date("d-m-Y");
				$index = 0;
				$limit = 50;
				$d['resultset']['materi'] 	= $this->m_kbm_materi->browse($index, $limit);
				$d['resultset']['evaluasi'] = $this->m_kbm_evaluasi->browse($index, $limit);
				// $this->_dump();
	...
	public function kartu($id = 0, $output = 'pdf', $kurikulum = 'k13')
	{
		$id = str_replace(".pdf", "", $id);

		$this->_set('data/profil/siswa/cover/k13');
		$this->_rowset('m_dprofil_siswa', $id, 'data/profil/siswa');
		$this->m_dprofil_siswa->rowsub($id);
	...
	public function print_excel_siswa()
	{
		$this->_set('data/profil/siswa/print_excel_siswa');

		$d = & $this->d;

		if ($d['user']['role'] != 'admin')
		{
			return alert_error('Cetak Excel Siswa hanya dapat dilakukan melalui admin kurikulum.', 'data/profil/siswa');
		}
		
		return $this->m_dprofil_siswa->print_excel_siswa();

	}
	
	public function reset_password_back($id = 0)
	{
		$this->_set('data/profil/siswa/reset_password_back');
	...
	public function manual_reset_password_siswa($aksi = '')
	{
		$this->_set('data/profil/siswa/manual_reset_password_siswa');
		...
		if ($d['user']['role'] != 'admin')
		{
			return alert_error('Reset USER PASS Manual Siswa hanya dapat dilakukan melalui admin kurikulum.', 'data/profil/siswa/reset_password_siswa');
		}
		
		if ($format)
			return $this->m_dprofil_siswa->manual_reset_password_siswa_expor();

		if (!$format && $d['post_request'] && !$d['error'])
			if ($this->m_dprofil_siswa->manual_reset_password_siswa_impor())
				return redir("data/profil/siswa/reset_password_siswa");
