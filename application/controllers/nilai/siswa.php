...
class Siswa extends MY_Controller
{
	...
	public function __construct()
	{
		parent::__construct(array(
			...
			'nilai/siswa/impor_des_perkembangan_karakter'	 => array(
				'user'		 => array('admin', 'sdm'),
				'library'	 => 'form',
				'helper'	 => 'form',
				'request'	 => array(
			...
			'nilai/siswa/impor_cat_perkembangan_karakter'	 => array(
				'user'		 => array('admin', 'sdm'),
				'library'	 => 'form',
				'helper'	 => 'form',
			),
			...
			'nilai/siswa/impor_data_prestasi_bta'	 => array(
				'user'		 => array('admin', 'sdm'),
				'library'	 => 'form',
				'helper'	 => 'form',
				'request'	 => array(
		...
		$this->rapor_separate = array('sman9smg','sma_terbang','demo','smkpltarcisius','smk_pika');
		
		$this->rapor_separate_k13update = array('sman14smg','sman8smg','smaissa1smg',
		'sma_setiabudhi','sma_terbang',
		'smk_penerbangan','smk_nusaputera','sma_michael','smp_terbang','smp_yoannes',
		'smkn6smg','smk_pika');
...
*/
	...
	public function rapor_generator($id = 0, $mode_range = 100, $tipe_rapor = 'original')
	{
		...
		foreach($d['id_nilai_siswa']['data'] as $nilai_siswa )
		{
			...
			/// 2017
			foreach($this->rapor_separate_k13update as $dk){
				if((strtolower(APP_SCOPE)==$dk) && (($set_kurikulum_nama=='2017')||($set_kurikulum_nama=='k13ver2019')) )
				{	
					$jml_chek++;			
					$this->load->helper($set_pdf_helper.$set_kurikulum_nama.'_view_'.strtolower(APP_SCOPE));
					$this->load->helper($set_pdf_helper.$set_kurikulum_nama.'_load_'.strtolower(APP_SCOPE));
			...
			$d['org_result'] = $this->m_nilai_siswa->subrow_org($id);*/
			$d['id_nilai_siswa']['data'][0]						= $this->m_nilai_siswa->rowset($nilai_siswa['id']);
			...
			if($tipe_rapor == "kkm"){
				$abre_filename = $abre_filename."_kkm";
			}elseif($tipe_rapor == "ttd"){
				$abre_filename = $abre_filename."_ttd" ;
			}else{
				$abre_filename = $abre_filename ;
			}

			$body = $this->_view($abre_filename, TRUE);
			
			  $mpdf = new mPDF();
			$mpdf->setAutoTopMargin = TRUE;
			$mpdf->setAutoBottomMargin = TRUE;
			$mpdf->WriteHTML($body);
			$mpdf->Output(APP_ROOT."content/generate_rapor/".APP_SCOPE
			."/".$d['row']['siswa_nis'].".pdf", 'F');
			unset($mpdf);
			unset($body);
			// }
	...
	public function skhun($id = 0, $mode_range = 100, $dump = '')
	{
	...
	public function skhun_kelas($id = 0, $mode_range = 100, $more = '')
	{
...
*/
	...
	public function mutasi_kelas($id = 0, $output = 'pdf')
	{
		$id = str_replace(".pdf", "", $id);
	...
	public function impor_des_perkembangan_karakter($aksi = '')
	{
		$this->_set('nilai/siswa/impor_des_perkembangan_karakter');
		...
		if ($format && $d['request']['semester_id'])
			return $this->m_nilai_siswa->impor_des_perkembangan_karakter_format();
		
		if ($format && $d['post_request'])
			return $this->m_nilai_siswa->impor_des_perkembangan_karakter_format();

		if (!$format && $d['post_request'] && !$d['error'])
			if ($this->m_nilai_siswa->impor_des_perkembangan_karakter())
				return redir("nilai/siswa");
		//$this->_dump();
		$this->form->set();
		$this->_view();
	...
	public function impor_cat_perkembangan_karakter($aksi = '')
	{
		$this->_set('nilai/siswa/impor_cat_perkembangan_karakter');
		...
		if ($format && $d['post_request'])
			return $this->m_nilai_siswa->impor_cat_perkembangan_karakter_format();

		if (!$format && $d['post_request'] && !$d['error'])
			if ($this->m_nilai_siswa->impor_cat_perkembangan_karakter())
				return redir("nilai/siswa");
		//$this->_dump();
		$this->form->set();
		$this->_view();
	...
	public function impor_data_prestasi_bta($aksi = '')
	{
		$this->_set('nilai/siswa/impor_data_prestasi_bta');
		...
		if ($format && $d['request']['semester_id'])
			return $this->m_nilai_siswa->impor_data_prestasi_bta_format();
		
		if ($format && $d['post_request'])
			return $this->m_nilai_siswa->impor_data_prestasi_bta_format();

		if (!$format && $d['post_request'] && !$d['error'])
			if ($this->m_nilai_siswa->impor_data_prestasi_bta())
				return redir("nilai/siswa");
		//$this->_dump();
		$this->form->set();
		$this->_view();
