...
class Siswa_absensi extends MY_Controller
{
	...
	public function __construct()
	{
		...
		parent::__construct(array(
			...
			'nilai/siswa_absensi/form_mutasi' => array(
				'user'		 => array('admin', 'sdm'),
				'library'	 => 'form',
				'helper'	 => 'form',
				'validasi'	 => array(
	...
	public function form_mutasi()
	{
		...
		if (!$d['admin'])
		{
			return alert_error("Anda tidak dapat mengubah lembar mutasi.", TRUE);
		}


		$this->_set('nilai/siswa_absensi/form_mutasi');
		$this->form->init('m_nilai_siswa', 'nilai/siswa');

		if ($d['post_request'] && !$d['error'])
		{
			if ($this->m_nilai_siswa->save('mutasi'))
			{
				$redir = "nilai/siswa/id/{$d['row']['id']}";
	...
	public function download_rapor($id, $nis, $tipe=''){
		...
		if($tipe!=''){
			...
			if($tipe=='oxf')
			{	$first = strtoupper($tipe).'_';	}
		
			$tipe = '_'.$tipe;
		}

		$file = base_url()."content/generate_rapor".$tipe."/".APP_SCOPE."/".$first.$nis.".pdf";
		...
		if($exists==true){
			...
			header("Content-Disposition: attachment; filename=" . urlencode("RAPOR".$tipe.$nis.".pdf"));   
			header("Content-Type: application/octet-stream");
