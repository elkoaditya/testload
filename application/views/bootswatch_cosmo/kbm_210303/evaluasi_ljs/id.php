			...
			<div id="tabel">
				...
				foreach ($butir_result['data'] as $idx => $butir):
					...
					if(isset($butir['pilihan']))
					{
						...
						$butir['opsi'] = $butir['pilihan']['pengecoh'];
						$butir['opsi'][$butir['pilihan']['kunci']['index']] =  $butir['pilihan']['kunci']['label']; 
						ksort($butir['opsi']);
					}
					...
					if (($user['role'] == 'admin') OR ($user['role'] == 'sdm') OR (($user['role'] == 'siswa') && ($evaluasi['show_kunci']==1)) OR $evaluasi['closed'])
						echo "<div><b>Poin: </b>{$butir['jwb_poin']} / {$butir['poin_max']}</div>";
						
						echo "<div><b>Koreksi Guru:</b>";
						echo ul(array($butir['jwb_koreksi']));
						echo "</div>";
						
					// jawaban
					if (($user['role'] == 'admin') OR ($user['role'] == 'sdm') OR (($user['role'] == 'siswa') && ($evaluasi['show_kunci']==1)) OR $evaluasi['closed'] OR $evaluasi['pilihan_jml']==0 )
					{
						echo "<br><br><div><b>Jawaban:</b>";
					...
					if ($butir['pilihan'] && ( ($user['role'] == 'admin') OR ($user['role'] == 'sdm') OR (($user['role'] == 'siswa') && ($evaluasi['show_kunci']==1)) OR $evaluasi['closed'] )):
					...
					if ( ($butir['type']==2) && ( ($user['role'] == 'admin') OR ($user['role'] == 'sdm') OR (($user['role'] == 'siswa') && ($evaluasi['show_kunci']==1)) OR $evaluasi['closed'] )):
						
						$kunsian = 1;
						while($kunsian<=9){
							$butir['kunci_isian'.$kunsian] 		= str_replace('<img src="/','<img src="'.base_url(),$butir['kunci_isian'.$kunsian]);
							$butir['kunci_isian'.$kunsian] 		= str_replace('<img src="../../','<img src="'.base_url(),$butir['kunci_isian'.$kunsian]);
							
							echo "<div><b>Alternatif Kunci Jawaban ke ".$kunsian.":</b> ";
							echo "&nbsp;".$butir['kunci_isian'.$kunsian]."<br></div>";
