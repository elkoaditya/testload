...
if(($author_ybs == 13)&&(APP_SCOPE == "andalus")){
	...
	$pills_kanan['komentar_group_all'] = array(
		...
		if($vidcall==1){ 
			...
			<div class="container">
			  <div class="row">
				...
				<?php
				if($user['role']=='siswa'){
					$video = VIDCALL.'/'.APP_SCOPE.'_'.$row["pelajaran_id"].'#interfaceConfig.TOOLBAR_BUTTONS='.
					...
					$video = VIDCALL.'/'.APP_SCOPE.'_'.$row["pelajaran_id"].'#config.disableDeepLinking=true&userInfo.displayName="'.$user["nama"].'"';
				}
				//$video = VIDCALL.'/'.APP_SCOPE.'_kode'.$row["pelajaran_id"];
				?>
				<h3>Kode Room untuk Aplikasi JITSI di HP<br>
				<b><?=APP_SCOPE."_".$row['pelajaran_id']?></b></h3>
				<a href='<?=VIDCALL?>/<?=APP_SCOPE."_".$row['pelajaran_id']?>' class='btn btn-info' target="_blank">KLIK JOIN VIDEO CALL by HP <br>*install dahulu jitsi aplikasi</a><br>
				<iframe allow="camera; microphone; fullscreen; display-capture" 
					style="width:100%; height:480px;" src='<?=$video?>'>
					<!--src='<?=VIDCALL?>/<?=APP_SCOPE."_".$row['pelajaran_id']?>#config.disableDeepLinking=true&userInfo.displayName="<?php echo $user['nama']; ?>"'>-->
