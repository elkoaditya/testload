...
function display_nama($row)
{
	...
	if ($deletable)
		$html .= a("kbm/bank_soal/delete?id={$row['id']}", 'Delete', 'class="btn btn-small btn-warning"') . '&nbsp;';
	
	if ($editable)
		$html .= a("kbm/bank_soal/form?id={$row['id']}", 'Edit', 'class="btn btn-small btn-success"');
	
	 //$a = a("kbm/bank_soal/form?id={$row['id']}", $row['pertanyaan'], 'title="lihat detail bank soal" ');
	//$a .= "<br/>";
	$html .= '</div>';
	...
	// FIX URL GAMBAR
	$row['pertanyaan'] 					= str_replace('src="../../','src="'.base_url(),$row['pertanyaan']);
	$row['pilihan']['kunci']['label']	= str_replace('src="../../','src="'.base_url(),$row['pilihan']['kunci']['label']);
	$row['pilihan']['pengecoh']			= str_replace('src="../../','src="'.base_url(),$row['pilihan']['pengecoh']);
	...
	if($row['author_nama']=='' && $row['author_id']>0){
		$row['author_nama'] = 'ADMIN';
	}
	$html .= "<div>
		<table>
			<tr>
				<td><b>Pembuat:</b> {$row['author_nama']} </td>
				<td><b>Tanggal</b> : {$row['registered']}</td>
			</tr>
			<tr>
				<td><b>Terakhir Di Ubah Oleh:</b> </td>
				<td><b>Tanggal</b> :{$row['modified']}</td>
			</tr>	
		</table>
		</div>";
...
//if ($mengajar_list):
	$pills[] = array(
		'label'	 => '<i class="icon-plus"></i>Susun Bank Soal Baru',
...
if ($user['role'] == 'admin'):	
	$pills[] = array(
		'label'	 => '<i class="icon-download"></i>Laporan Guru Input',
		'uri'	 => "kbm/bank_soal/laporan_guru_input",
		'attr'	 => 'title="laporan guru input"',
		'class'	 => 'active'
	);
endif;	
...
$input = array(
		...
		'author_id' => array(
				...
				'name' => 'author_id',
				'id' => 'author_id',
				'extra' => 'id="author_id" class="input input-medium select"',
				'options' => array(
						//'' => 'semua',
						$user['id'] => $user['nama'],
						),
...
if ($user['role'] == 'admin'):
	...
	$input['kategori_id']['options'] = $this->m_option->kategori_mapel_user('kategori', 'evaluasi');
...
$bar = '<div>'
			...
			. form_cell($input['author_id'], $request) . '&nbsp;';
if ($admin):
