		...
		foreach ($resultset['data'] as $data):
			...
			if(($kelas_nama != $data['kelas_nama'])||(($jml%18)==1)){
				...
				echo '<tr>';
					...
					echo '<td class="head-2" width="18%">No Ujian</td>';
			...
			$image_absensi = str_replace("data:image/png;base64,","",$data['image_absensi']);
			$image_absensi = base64_decode($image_absensi);
			$im = imagecreatefromstring($image_absensi);
			if($im !== false){
			...
			echo '<tr>';
				...
				echo '<td class="info" align="center">'.$data['siswa_no_un'].'</td>';
				echo '<td class="info" align="center">'.$data['waktu_absensi'].'</td>';
				echo '<td class="info" valign="top" rowspan="2" width="15%" >'.$jml.'.<br> 
				
				<img width="80px" src="'.$data['image_absensi'].'" alt="Red dot"></td>';
