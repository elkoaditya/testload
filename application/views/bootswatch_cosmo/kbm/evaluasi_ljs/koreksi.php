  ...
  <link href="<?php echo base_url();?>assets/jquery_image_viewer/css/docs.css" rel="stylesheet">-->
				...
				foreach ($butir_result['data'] as $idx => $butir):
					...
					if ($butir['pilihan']):
						...
						echo "<b>Poin: </b>";
						echo form_cell($input_poin, $butir) . "  {$butir['jwb_poin']} ";
						echo "</div>" . NL;
						...
						//ISIAN
						if($butir['type']==2){
							$input_poin['options'][$butir['poin_min']] = $butir['poin_min'];
							$input_poin['options'][$butir['poin_max']] = $butir['poin_max'];
						}else{
						...
						$input_koreksi = array(
								...
								'name' 	=> $name_koreksi,
								'id' 	=> $name_koreksi,
								'value' => (($post_request) ? $this->input->post($name_koreksi) : $butir['jwb_koreksi']),
								'class' => 'input tinymce_soal',
								'style' => 'height: 200px;',
						);
						
						echo "<div><b>Koreksi Guru:</b>";
						echo form_cell($input_koreksi, $butir);
						echo "</div>";	
					endif;
