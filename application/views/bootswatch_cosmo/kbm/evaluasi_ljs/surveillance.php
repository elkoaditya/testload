...
function display_ttd_absensi($row)
{
	if(isset($row['image_absensi'])){
		return ' <div><img src="'.$row['image_absensi'].'" alt="Red dot"></div> ';
	}else{
		return '<div style="color:red;"><b><i>Belum TTD Absen</i></b></div> ';
	}
}
...
function display_action($row, $evaluasi, $user, $admin)
{
	$author_ybs = (($user['id'] == $evaluasi['author_id'])||($user['id'] == $evaluasi['pelajaran_guru_id']));
...
if($absensi_evaluasi == 'enable'){
	$pills[] = array(
			'label'	 => '<i class="icon-download"></i>Download Absensi',
			'uri'	 => "kbm/evaluasi_ljs/download_absensi?".$get_request_uri,
...
if($admin){
	...
	$pills[] = array(
			'label'	 => '<i class="icon-download"></i>Download LJS',
...
if($absensi_evaluasi == 'enable'){
	...
	$table['data']['TTD Absensi']	= array(FALSE, 'display_ttd_absensi') ;
}
//////////////////////////////////////

$table['data']['Nilai']	 	= array(FALSE, 'display_nilai');
$table['data']['Status']	= array(FALSE, 'display_status');
$table['data']['Action']	= array(FALSE, 'display_action', $evaluasi, $user, $admin);
