...
function js_send_text($id, $no_soal, $pengerjaan_ljs, $soal, $simulasi){
	?>
	<script type="text/javascript">
...
<html lang="en">
 ...
 <!--<body translate="no" class="preview" id="top" data-spy="scroll" data-target=".subnav" data-offset="80" oncontextmenu="return false" ondragstart="return false" onselectstart="return false">-->
	<?php //}else{ ?>
  <body translate="no" class="preview" id="top" data-spy="scroll" data-target=".subnav" data-offset="80">
	<?php //} ?>
   ...
   if($this->input->get('open_soal')==1)
	{}
	   ...
	   <div class="loader">
		<a href="http://<?=$_SERVER['HTTP_HOST']?><?=$_SERVER['REQUEST_URI']?>&open_soal=1" class="btn btn-block"> 
				...
				<div class="container">
				  <div class="row">
					...
					<iframe allow="geolocation; microphone; camera" 
						...
						src='<?=VIDCALL?>/<?=APP_SCOPE."_kelas_".$kelas_id?>#config.disableDeepLinking=true&userInfo.displayName="<?php echo $user['nama']; ?>"'>
					</iframe>
			...
			<div id="tabel">
				...
				<div class="prototype-wrapper"> <!-- A wrapper to emulate use in a webpage and center align -->
					...
					if($soal['type']==3){
						...
						js_send_text($input_essay['id'], $no_soal, $pengerjaan_ljs, $soal, $this->input->get("simulasi"));
					//// ISIAN
					}else if($soal['type']==2){
						...
						if ($evaluasi['pilihan_jml'] > 1 && is_array($soal['opsi'])):
							...
							foreach ($soal['opsi'] as $key => $opsi):
								if(strtolower($evaluasi['metode']) == "opsi_tampil_abc"){ 
									...
									//$opsi = str_replace("<div>","<div>".strtoupper($key).". ",$opsi);
								}
								$name = "butir-{$soal['id']}";
							...
							new FroalaEditor('textarea#<?=$input_essay['id']?>', {
								...
								saveParams: {
									ljs_id:'<?php echo $pengerjaan_ljs["id"];?>',
									soal_id:'<?=$soal["id"]?>',
									simulasi:'<?=$this->input->get("simulasi")?>' 
								},
								...
								// Allow to upload MP4, WEBM and OGG
								videoAllowedTypes: ['wmv', 'webm', 'mp4', 'ogg', 'avi', 'flv', '3gp', 'mkv', 'mpg', 'swf', 'dat', 'mov', 'vob', 'nut', 'hevc'],
								...
								events: {
									 'save.before': function () {
									  ...
									  'save.after': function (j) {
											...
											$('#soal<?=$no_soal?>').html('<div style="background-color:#00AAFF">&nbspPertanyaan :: <?=$no_soal?> <br>( Jawaban tersimpan )<div>');
											if(no_jawab[<?=$no_soal?>] == 0 )
											{	
												jml_jawab++;
												no_jawab[<?=$no_soal?>] = 1;
									  ...
									  'save.error': function (j) {
											...
											$('#soal<?=$no_soal?>').html('<div style="background-color:#FF4444">&nbspPertanyaan :: <?=$no_soal?> ( Jawaban gagal di simpan, coba beberapa saat lagi. )<div>');
											$('#nomer_soal<?=$no_soal?>').html('<table>'+
											 '<tr>'+
											  '<td width="50px" height="50px" bgcolor="#FF4444" align="center">'+
												<?=$no_soal?>+
				...
				if ($user['role'] == 'siswa'):
					...
					. ' <i class = "icon-save icon-white"></i> SELESAI PENGERJAAN <br><i>*klik jika yakin <br>selesai menjawab semua</i></button> '
					. ' </div>';
					echo '</fieldset>';
...
<script>
		...
		var validate = $('#soal'+$no);
		  ...
		  this.timer = setTimeout(function () {
			$.ajax({
			  ...
			  data: 'ljs_id=<?php echo $pengerjaan_ljs['id'];?>&soal_id='+$soal_id+'&pilihan=' + $pilihan+'&poin=' + $poin+'&simulasi=<?=$this->input->get('simulasi')?>',
			  dataType: 'json',
			  type: 'POST',
			  success: function (j) {
				
				validate.html('<div style="background-color:'+j.warna+'">&nbspPertanyaan :: '+$no+' <br>( '+j.message+' )<div>');
				if(no_jawab[$no] == 0 )
				{	
					jml_jawab++;
					no_jawab[$no] = 1;
		...
		var validate = $('#soal'+$no);
		  ...
		  this.timer = setTimeout(function () {
			$.ajax({
			  ...
			  data: 'ljs_id=<?php echo $pengerjaan_ljs['id'];?>&soal_id='+$soal_id+'&jawaban=' + $jawaban+'&simulasi=<?=$this->input->get('simulasi')?>',
			  dataType: 'json',
			  type: 'POST',
			  success: function (j) {
	...
	$(window).load(function() {
		...
		var x	= document.getElementsByClassName("fr-wrapper");
		/*
				var x	= document.getElementsByClassName("fr-wrapper");
				...
				while(!(typeof x[i] === 'undefined')) {
					...
					//document.write(str);
					var str = str.replace('a href="https://www.froala.com/wysiwyg-editor?k=u"', 'div');
					var str = str.replace('</a>', '</div>');
					
					var str = str.replace("https://www.froala.com/wysiwyg-editor?k=u", "");
					
					var str = str.replace("#EF5350","#41bb19");
					var str = str.replace("Unlicensed copy of the Froala Editor. Use it legally by purchasing a license.", "Tulis Jawaban Anda.");
					x[i].innerHTML = str;
