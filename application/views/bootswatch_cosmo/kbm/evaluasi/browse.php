...
// function
//print_r($resultset);
function display_nama_kesiswa($row, $absensi_evaluasi)
{
	$set_metode = "form";
	 $a = $row['nama'];
	...
	$evaluasi_mulai = strtotime($row['evaluasi_mulai']);
	$evaluasi_tutup = strtotime($row['evaluasi_ditutup']);
	...
	if($trigger==0){	
		...
		$kerjakan = '<div align="left">' . a("kbm/evaluasi_ljs/".$set_metode."?id={$row['id']}", '<i class="icon-pencil"></i> Kerjakan', 'class="btn btn-success" title="kerjakan evaluasi ini"') . '</div>';
		
		
		if($absensi_evaluasi == 'enable'){
	
			if($row['waktu_absensi']==''){
				$id = $row["id"];
				$a .= '<div id="status_kerjakan_'.$row['id'].'"></div>'.
				'<div id="kerjakan_'.$row['id'].'"><button class="btn btn-warning" onclick="modalAbsensi('.$id.' , '.$evaluasi_mulai.' , '.$evaluasi_tutup.')"><i class="icon-tag"></i> Absensi </button></div>';
			}else{
				$a .= $kerjakan;
			}
		}else{
...
function display_bentuk($row)
{
	$result = '';
	 
	if($row['pilihan_jml'] > 1){ 
		$result = 'pilihan';
		if($row['plus_isian']==1){
			$result .= ' + isian';
		}
...
function display_kisi_kisi($kisi_kisi)
{
	if($kisi_kisi==1)
	{	$cetak = '<div style="color:green;">Masuk Kisi-kisi</div>';	}
	else if($kisi_kisi==0)
	{	$cetak = '<div style="color:red;">Tidak Masuk Kisi-kisi</div>';		}
...
$table = array(
	...
	'data'				 => array(
		...
		'Bentuk'						 => array(FALSE, 'display_bentuk',),
...
if ($user['role'] == 'siswa'):
	$table['data']['Nama Evaluasi'] = array(FALSE, 'display_nama_kesiswa', $absensi_evaluasi);
	...
	$table['data']['<div align="center">Kisi-kisi</div>'] = array('kisi_kisi', 'display_kisi_kisi');
...
if ($user['role'] != 'siswa'):
	$bar .= form_cell($input['kisi_kisi'], $request) . '&nbsp;';
...
<html lang="en">
	...
	<body class="preview" id="top" data-spy="scroll" data-target=".subnav" data-offset="80">
		...
		<?php 
			...
			if($absensi_evaluasi=='enable'){
				$this->load->view (THEME .'/kbm/evaluasi/modal_absensi');
			}
