...
function display_pertanyaan($row, $editable, $deletable, $uri, $evaluasi) { 
	...
	$param = "id={$row['id']}".$param_isian."&posisi_kd={$row['posisi_kd']}&redir={$uri}/{$row['evaluasi_id']}";
	...
	// KETERANGAN TAMBAHAN
	$html .= "<div><b>Kompetensi Dasar:</b> {$row['type_kd']} {$row['ket_type_kd']}</div>";
	$html .= "<div><b>Indikator:</b> {$row['indikator']}</div>";
...
// pills link
$set_form = "form_abc"; 
//if(strtolower($row['metode']) == "opsi_tampil_abc"){ 
//			$set_form = "form_abc"; 
//}
/*
...
while($tampil_kd<=$jumlah_kd){
	...
	$detail['tambahan'] = array(
		...
		'Bobot Nilai'	 	 => array('bobot_nilai', 'display_bobot_nilai', $resultset[$tampil_kd]['total_rows']),
	);
	...
	$pills_tambah[$tampil_kd][] = array(
			...
			'uri' => "kbm/evaluasi_soal/".$set_form."?evaluasi_id={$row['id']}&posisi_kd={$tampil_kd}&redir={$uri}/{$row['id']}",
			'attr' => 'title="tambah butir soal baru" ',
			'class' => 'active',
	);
...
if($row['plus_isian']==1){
	...
	$pills_tambah[$tampil_kd][] = array(
			...
			'uri' => "kbm/evaluasi_soal/".$set_form."?evaluasi_id={$row['id']}&isian=1&posisi_kd={$tampil_kd}&redir={$uri}/{$row['id']}",
			'attr' => 'title="tambah butir soal baru" ',
			'class' => 'active',
	);
	...
	endforeach;
	echo "<div class=\"control-group\"><div class=\"controls\">"
		.a("kbm/evaluasi_pembagian_kd/form?evaluasi_id={$row['id']}&isian=1&posisi_kd={$tampil_kd}",
			'Edit', 'class="btn btn-small btn-warning"') . '&nbsp;';
	echo '</fieldset></div><br/>';
...
if($row['plus_uraian']==1){
	...
	$pills_tambah[$tampil_kd][] = array(
			...
			'uri' => "kbm/evaluasi_soal/".$set_form."?evaluasi_id={$row['id']}&uraian=1&posisi_kd={$tampil_kd}&redir={$uri}/{$row['id']}",
			'attr' => 'title="tambah butir soal baru" ',
			'class' => 'active',
	);
	...
	endforeach;
	echo "<div class=\"control-group\"><div class=\"controls\">"
		.a("kbm/evaluasi_pembagian_kd/form?evaluasi_id={$row['id']}&uraian=1&posisi_kd={$tampil_kd}",
			'Edit', 'class="btn btn-small btn-warning"') . '&nbsp;';
	echo '</fieldset></div><br/>';
	
	// ARSIP URAIAN LAMA
	
	if($row['detail_uraian']!=''){
		$base_url = base_url();
