...
$edit_skor = (($author_ybs OR $admin) && !$row['closed'] && $row['semester_id'] == $semaktif['id']);
$deletable = ($edit_skor && !$row['published']);
...
function display_pertanyaan($row, $edit_skor, $deletable, $uri) {
	soal_prepare($row);
	...
	if ($edit_skor)
		$html .= a("kbm/evaluasi_soal/skor_soal?evaluasi_id={$row['id']}&redir={$uri}/{$row['evaluasi_id']}", 'Ubah Skor Soal', 'class="btn btn-small btn-success"');
	else
	...
	// FIX URL GAMBAR
	// $row['pertanyaan'] 					= '<pre>'.print_r(json_encode($row['pertanyaan'])).'</pre>';
	...
	if($row['bank_soal_id']>0){
		...
		$kesukaran_id = json_decode($row['pertanyaan']);
	// sumber
...
$pills[] = array(
		'label' => '<i class="icon-star"></i>TAMBAH SOAL',
		'uri' => "kbm/evaluasi_soal/form3?evaluasi_id={$row['id']}&redir={$uri}/{$row['id']}",
...
$table = array(
		...
		'data' => array(
				'Pertanyaan' => array(FALSE, 'display_pertanyaan', $edit_skor, $deletable, $uri),
		),
);
