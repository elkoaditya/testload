...
foreach ($config_value['data'] as $key => $value) {
	...
	if($value['grade_kelas']==12){
		...
		$input_kelulusan[$key] = array(
			'config_value',
			'label'	 => $value['nama_kelas'],
			'type'	 => 'input',
			'name'	 => 'config_value_kelulusan['.$value['nilai_kelas_id'].']',
			'id'	 => 'config_value_kelulusan['.$value['nilai_kelas_id'].']',
			'value'	 => $value['tanggal_kelulusan'],
		);
	}
	$jml_key++;
}
	...
	<body class="preview" id="top" data-spy="scroll" data-target=".subnav" data-offset="80">
		...
		<div class="container">
			...
			///	echo '</pre>';
				...
				echo '<fieldset> KELULUSAN';
	
				foreach ($input_kelulusan as $key => $value) {
					echo div('class="control-group"');
					echo "<label class=\"control-label\" for=\"{$value['id']}\">{$value['label']}</label>";
					echo div('class="controls"', form_cell($value));
					echo "</label></div>" . NL;
