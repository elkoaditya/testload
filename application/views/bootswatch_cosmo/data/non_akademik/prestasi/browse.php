...
function display_action($row) {
	return a("data/non_akademik/prestasi/form?id={$row['id']}", 'edit', 'title="ubah data prestasi ini"');
}

function display_nama($row) {
	return a("data/non_akademik/prestasi/id/{$row['id']}", $row['nama'], 'title="lihat detail data prestasi ini"');
}
/*
...
if ($admin):
	$pills[] = array(
			'label' => '<i class="icon-star"></i>Tambah Prestasi',
...
$table = array(
		...
		'data' => array(
				...
				'Kegiatan Prestasi' => 'kegiatan_prestasi_nama',
		),
);
...
<html lang="en">
	...
	<?php $this->load->view(THEME . "/-html-/head", array('title' => 'Prestasi')); ?>
