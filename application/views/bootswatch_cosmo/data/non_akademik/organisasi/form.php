...
//
//$admin = cfguc_admin('akses', 'data', 'akademik', 'organisasi');
//
// breadcrumbs
...
if ($row['id'] > 0):
	$breadcrumbs["#{$row['id']}"] = "data/non_akademik/organisasi/id/{$row['id']}";
	$breadcrumbs[] = "#edit";
else:
	$breadcrumbs[] = "#baru";
endif;
...
if ($row['id'] > 0)
	$pills[] = array(
			'label' => '<i class="icon-list-alt"></i>Detail Organisasi',
			'uri' => "data/non_akademik/organisasi/id/{$row['id']}",
			'attr' => 'title="kembali ke detail organisasi"',
	);
...
if ($row['id'] > 0)
	$btn_back = a("data/non_akademik/organisasi/id/{$row['id']}", ' <i class="icon-circle-arrow-left icon-white"></i> Kembali ke detail organisasi', 'class="btn btn-info "');
else
	$btn_back = a("data/non_akademik/organisasi", ' <i class="icon-circle-arrow-left icon-white"></i> Kembali ke tabel kelas', 'class="btn btn-info "');
?>
<!DOCTYPE html>
<html lang="en">

	<?php $this->load->view(THEME . "/-html-/head", array('title' => 'Form Organisasi')); ?>
