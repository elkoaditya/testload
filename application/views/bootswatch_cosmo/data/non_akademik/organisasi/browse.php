...
function display_action($row) {
	return a("data/non_akademik/organisasi/form?id={$row['id']}", 'edit', 'title="ubah data organisasi ini"');
}

function display_nama($row) {
	return a("data/non_akademik/organisasi/id/{$row['id']}", $row['nama'], 'title="lihat detail data organisasi ini"');
}
...
if ($admin):
	$pills[] = array(
			'label' => '<i class="icon-star"></i>Tambah Organisasi',
			'uri' => "data/non_akademik/organisasi/form",
			'attr' => 'title="tambah organisasi baru"',
	);
endif;
...
<html lang="en">
	...
	<?php $this->load->view(THEME . "/-html-/head", array('title' => 'Organisasi')); ?>
