...
function display_title($row) {
	return a("nilai/organisasi/id/{$row['id']}", $row['org_nama'], 'title="lihat detail nilai organisasi"');
}
...
foreach ($org_terkait as $xid => $xnama)
	$input['org_id']['options'][$xid] = $xnama;
...
$table = array(
		...
		'data' => array(
				...
				'Organisasi' => array(FALSE, 'display_title'),
...
$bar = '<div>'
		...
		. form_cell($input['org_id'], $request) . '&nbsp;	'
...
<html lang="en">
	...
	<?php $this->load->view(THEME . "/-html-/head", array('title' => 'Keterangan Organisasi')); ?>
