...
$breadcrumbs = array(
	...
	'#impor penilaian ekstrakurikuler'
);

// pills link

$pills[] = array(
	'label'	 => '<i class="icon-table"></i>Nilai Ekstrakurikuler',
	'uri'	 => 'nilai/ekstrakurikuler',
	'attr'	 => 'title="kembali ke daftar nilai ekstrakurikuler"',
);
$pills[] = array(
	'label'	 => "<i class=\"icon-table\"></i>Nilai {$row['ekskul_nama']}",
	'uri'	 => "nilai/ekstrakurikuler/id/{$row['id']}",
	'attr'	 => 'title="kembali ke detail nilai ekstrakurikuler"',
);
$opsi_kelas = $this->m_option->kelas();

// buttons

$btn_back = a("nilai/ekstrakurikuler/id/{$row['id']}", "<i class=\"icon-circle-arrow-left icon-white\"></i> Kembali ke daftar nilai ekstrakurikuler ", 'class="btn btn-info "');

?>
<!DOCTYPE html>
<html lang="en">

<?php $this->load->view(THEME . "/-html-/head", array('title' => 'Impor Nilai Ekstrakurikuler')); ?>
		...
		<div class="container">
			...
			<div id="tabel">
				<div class="page-header">
					<h1>Impor Nilai Ekskul :: <?php echo $row['ekskul_nama']; ?></h1>
				</div>
				...
				echo form_openmp("nilai/ekstrakurikuler/expor/{$row['id']}", 'class="form-horizontal well"');
