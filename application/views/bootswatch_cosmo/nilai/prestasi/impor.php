...
$breadcrumbs = array(
		...
		'#impor penilaian prestasi'
);

// pills link

$pills[] = array(
		'label' => '<i class="icon-table"></i>Nilai Prestasi',
		'uri' => 'nilai/prestasi',
		'attr' => 'title="kembali ke daftar nilai prestasi"',
);
$pills[] = array(
		'label' => "<i class=\"icon-table\"></i>Nilai {$row['prestasi_nama']}",
		'uri' => "nilai/prestasi/id/{$row['id']}",
		'attr' => 'title="kembali ke detail nilai prestasi"',
);
$opsi_kelas = $this->m_option->kelas();

// buttons

$btn_back = a("nilai/prestasi/id/{$row['id']}", "<i class=\"icon-circle-arrow-left icon-white\"></i> Kembali ke daftar nilai prestasi ", 'class="btn btn-info "');
?>
<!DOCTYPE html>
<html lang="en">

	<?php $this->load->view(THEME . "/-html-/head", array('title' => 'Impor Nilai Prestasi')); ?>
    ...
    <div class="container">
			...
			<div id="tabel">
				<div class="page-header">
					<h1>Impor Nilai Prestasi :: <?php echo $row['prestasi_nama']; ?></h1>
				</div>
				...
				echo form_openmp("nilai/prestasi/expor/{$row['id']}", 'class="form-horizontal well"');
