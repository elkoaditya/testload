...
if ($user['role'] == 'siswa'):
	...
	$pills_kanan = array(
		...
		'download'		 => array(
			...
			'attr'	 => 'title="ubah data siswa ini" target="_blank"',
			'class'	 => 'disabled',
		),
		'simulasi'	 => array(
...
$detail['umum'] = array(
	...
	'Tambah Uraian'		 => array('plus_uraian', array('<span style="color:#F00;">tidak</span>', '<span style="color:#00F;">ya</span>')),
...
function display_webcam($row)
{
	$kelas_nama = strtolower(str_replace(" ","_",$row['kelas_nama']));
	return $a='<a href='.VIDCALL.'/'.APP_SCOPE.'_kelas_'.$kelas_nama.'#config.disableDeepLinking=true&userInfo.displayName="'.$_COOKIE["nama"].'" 
	class="btn btn-success" title="WebCAM Kelas" target="_blank"> WebCAM Kelas</a> ';
}
...
if ($user['role'] != 'siswa'):
	...
	if($row['published']){
		$table['data']['WebCAM'] = array(FALSE, 'display_webcam');
