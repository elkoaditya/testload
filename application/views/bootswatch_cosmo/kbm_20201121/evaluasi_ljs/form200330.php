	...
	?>
			...
			<div id="tabel">
				...
				<div class="prototype-wrapper"> <!-- A wrapper to emulate use in a webpage and center align -->
					...
					$base_url = base_url();
					//if(APP_SUBDOMAIN){
						//$base_url = "http://45.114.118.225/".APP_SUBDOMAIN."/";
					//}
					...
					if ($evaluasi['pilihan_jml'] > 1 && is_array($soal['opsi'])):
						...
						<script type="text/javascript">
						tinymce.init({
							selector: "textarea#butir-<?=$soal['id']?>",
							setup: function (ed) {
								ed.on("change", function () {
									//TinyMceGetStatsLost(ed);
									input_jawab_essay('<?=$no_soal?>' ,'<?=$soal['id']?>', ed.getBody().innerHTML)
								})
							}
						});
				...
				if($evaluasi['plus_uraian']){
					...
					$evaluasi['detail_uraian']		= str_replace('<img src="../../','<img src="'.$base_url,$evaluasi['detail_uraian']);
