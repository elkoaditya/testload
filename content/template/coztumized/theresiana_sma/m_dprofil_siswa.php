...
class M_dprofil_siswa extends MY_Model
{
	...
	function edit_excel_siswa_expor()
	{
		...
		if (TRUE):
			$d = & $this->ci->d;
			$dm = & $this->dm;
			$excel_source = 'content/template/coztumized/' . strtolower(APP_SCOPE) . '/edit_excel_siswa.blank.xlsx';
			$siswa_query = array(
				'from'	 => 'dprofil_siswa siswa',
				'join'	 => array(
		...
		while($grade>=10):
			...
			*/
			...
			excel_security_cell_unlock($sheet, "I12:AH".$excel_row);
	...
	function tambah_excel_siswa_expor()
	{
		...
		if (TRUE):
			$d = & $this->ci->d;
			$dm = & $this->dm;
			$excel_source = 'content/template/coztumized/' . strtolower(APP_SCOPE) . '/tambah_excel_siswa.blank.xlsx';
		...
		for ($_sheet_index = 0; $_sheet_index < $sheet_jml; $_sheet_index++):
			...
			while($row_max>=$row_start):
				foreach ($dm['map.excel.siswa.tambah.impor'] as $_excel_col => $_db_col):
					...
					if($_db_col== 'kelas_id' && ($_row[$_db_col]=='' || ( filter_var($_row[$_db_col], FILTER_VALIDATE_INT) !== false ) ) )
					{	return alert_error('Kelas ID salah di kolom '.$_excel_col . $row_start); }
